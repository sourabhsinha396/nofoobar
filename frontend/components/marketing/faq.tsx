...
const faqs: ReadonlyArray<{ question: string; answer: string }> = [
  {
    ...
    answer:
      "Through Stripe Connect. Money flows directly to your Stripe account, on Stripe's standard payout schedule. We never sit between you and your revenue.",
  },
  {
    question: "Do you charge transaction fees on top of Stripe?",
    answer:
      "No. Stripe's standard fee (2.9% + 30¢ in the US, varies by country) is what you pay on a sale. Nofoobar takes nothing on payments. You only pay the monthly hosting subscription if you're on Starter or Pro.",
  },
  {
    question: "What's included in the AI features?",
    answer:
      "Starter includes 10 AI blog drafts per month. Pro removes that limit and adds the AI quiz generator. Both let you fully edit the output before publishing, so nothing ships without you signing off.",
  },
  {
        ...
        <Accordion type="single" collapsible className="w-full">
          {faqs.map((faq, i) => (
            <AccordionItem key={faq.question} value={`item-${i}`}>
              ...
              <AccordionContent className="pt-1 pb-7 text-base leading-relaxed text-muted-foreground md:pb-8 [&_a]:underline [&_a]:underline-offset-3 [&_a]:hover:text-foreground">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
